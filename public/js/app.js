...
class RootCausePowerApp {
            ...
            <div class="bg-white rounded-lg p-8 max-w-2xl mx-auto text-center shadow-2xl">
                ...
                <div class="bg-green-50 p-6 rounded-lg mb-6">
                    ...
                    <div class="flex flex-wrap justify-center gap-3">
                        <button onclick="app.playPanicAttackHelp(); app.closeCrisisOverlay();" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors">
                            🎵 Panic Attack Help Audio (3 min)
                        </button>
                        <button onclick="app.playGuidedGrounding(); app.closeCrisisOverlay();" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors">
                            🧘 Grounding Audio (8 min)
                        </button>
                        <button onclick="app.showSection('ptsd-corner'); app.closeCrisisOverlay();" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors">
                ...
                <div class="text-center">
                    <button onclick="app.closeCrisisOverlay()" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600 transition-colors mr-4">
                        I'm Safe for Now
                    </button>
                    <button onclick="app.createSafetyPlan(); app.closeCrisisOverlay();" class="bg-purple-600 text-white px-6 py-2 rounded hover:bg-purple-700 transition-colors">
                        Create Safety Plan
                    </button>
                </div>
    ...
    showCrisisResources() {
        ...
        // Also trigger enhanced crisis support
        setTimeout(() => {
            this.showEmergencyCrisisSupport();
        }, 2000);
    }
        ...
        const content = `
            <div class="flex flex-col h-full">
                ...
                <div class="flex space-x-2">
                    <input type="text" id="coach-user-input" data-coach-type="${coachType}" placeholder="Type your message or click mic to speak..." class="flex-1 p-3 border rounded-lg focus:ring-2 focus:ring-${coach.color}-500 focus:border-${coach.color}-500">
                    <button id="voice-input-btn" onclick="app.toggleVoiceInput('${coachType}')" class="bg-purple-500 text-white px-4 py-3 rounded-lg hover:bg-purple-600 transition-colors" title="Voice Input">
                ...
                <div class="text-xs text-gray-500 mt-2 text-center">
                    <span class="hidden md:inline">Press Enter to send • Click 🎙️ for voice chat</span>
                    <span class="md:hidden">Tap send or 🎙️ for voice</span>
                    <span class="block mt-1">${this.currentUser.plan === 'Free' ? 'Free users: limited daily interactions' : 'Unlimited interactions'}</span>
                </div>
            </div>
        `;
...
Root Cause Power - AI-Powered Trauma Recovery Platform
    ...
    playPanicAttackHelp() {
        this.playTherapeuticAudio('audio/panic-attack-help.wav', false);
    }
...
function initializeApp() {
    try {
        ...
        console.log('- logMood:', typeof app.logMood);
        console.log('- addDailyWin:', typeof app.addDailyWin);
        ...
        // Load and display daily wins
        app.dashboardData = app.loadData('dashboardData') || {};
        app.updateDailyWinsDisplay();
